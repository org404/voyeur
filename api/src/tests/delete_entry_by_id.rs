...
async fn test_bad() {
...

    {
        ...
        let (r1, r2, r3, r4, r5) = tokio::join!(
            client.delete("/api/v1/entries/-1")
                .header(Header::new("X-Namespace", "a"))
                .header(ContentType::JSON).dispatch(),
            client.delete("/api/v1/entries/0.12345")
                .header(Header::new("X-Namespace", "a"))
                .header(ContentType::JSON).dispatch(),
            client.delete("/api/v1/entries/a")
                .header(Header::new("X-Namespace", "a"))
                .header(ContentType::JSON).dispatch(),
            client.delete(format!("/api/v1/entries/{}", u64::MAX as u128 + 1))
                .header(Header::new("X-Namespace", "a"))
                .header(ContentType::JSON).dispatch(),
            client.delete("/api/v1/entries/0x1122334455")
                .header(Header::new("X-Namespace", "a"))
                .header(ContentType::JSON).dispatch()
        );
